...
// Professional gradient palettes: navy, emerald, gold, royal blue, ...
const BADGES: readonly (readonly [string, string])[] = [
  ["#22366b", "#0a1226"], // dark navy
  ["#12b886", "#065f46"], // emerald green
  ["#f2b431", "#a35f06"], // gold
  ["#3b74ff", "#1a3fb0"], // royal blue
  ["#7c5cff", "#3b1fa8"], // deep purple
  ["#16b8c8", "#0b5c6b"], // teal
  ["#e0345c", "#7a1030"], // crimson
  ["#5b6b86", "#1b2438"], // slate
];
...
export function generateInitialsSvg(seed: string, name: string): string {
  const [c0, c1] = BADGES[hash(seed + "|badge") % BADGES.length];
  const initials = initialsOf(name);
  const arabic = ARABIC.test(name);
  const font = `font-family="Arial, Helvetica, sans-serif" font-weight="700" fill="#fff" text-anchor="middle"`;
