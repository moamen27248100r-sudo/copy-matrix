-- QA found a third, pre-existing gap the 0171 tri-tier PnL retune never
-- touched: three separate "scalper trading_style" code paths that override
-- the risk_archetype-tier-based magnitude with a flat, tiny pip range --
-- completely independent of risk tier, so e.g. a scalper on the low tier
-- (needs 0.3%-2.5% moves) or even a scalper on the high tier could close
-- at ~0.02%-0.3%, far under its tier's floor. This is exactly the "too
-- small" outlier pattern (tp/sl/timeout/breakeven triggers, near-zero %)
-- still appearing after migrations 0171 and 0174 were both live.
--
-- Removed (trading_style should only drive trade FREQUENCY -- v_open_prob,
-- untouched here -- not PnL magnitude, which is risk_archetype's job per
-- the customer's explicit 3-tier spec):
--   1. CLOSE loop: `v_move := v_move * (0.15 + random()*0.15)` scalper-only
--      scale-down applied after the tier-based v_move was computed.
--   2. CLOSE loop: a flat 15-40 pip post-clamp on timeout/breakeven exits
--      for scalpers, overriding the tier-consistent v_move entirely.
--   3. OPEN block: a flat 8-28 pip override of the tier-based v_tp_pips/
--      v_sl_pips for scalpers, so their stored take_profit/stop_loss never
--      reflected their archetype's tier in the first place.
--
-- Redefined from the current live definition (0174); nothing else changed.
