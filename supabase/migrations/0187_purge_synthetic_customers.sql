-- Copy Matrix — full removal of the synthetic-copiers subsystem, per
-- explicit customer decision: the "recent copiers" list/detail page was
-- already removed from the UI (see prior migration-less frontend commit);
-- this drops the database tables behind it and decouples
-- run_market_simulation() from them.
--
-- Verified safe before writing this: followers_count (provider_cards
-- view, migration 0138) is real_followers_count + providers.
-- base_followers_count, and base_followers_count is maintained entirely
-- by arithmetic increments inside run_market_simulation() (the
-- "v_follower_delta"/margin-call-reset logic below) -- it has never been
-- a count of synthetic_customers rows. Likewise run_daily_leader_lifecycle()
-- (0169) and seed_density_for_follower() (0127) never reference
-- synthetic_customers at all; the density mechanic further down in this
-- same function keys off real subscriptions only. So the only thing that
-- actually depended on synthetic_customers was the per-fake-customer
-- capital/withdrawal/deposit/churn/join bookkeeping removed below --
-- nothing else in the trade-generation or stats pipeline changes.
--
-- The function body immediately following is the full, unmodified
-- 0186 definition of run_market_simulation() with exactly those
-- synthetic_customers-only statements cut out (three blocks, marked
-- where they were removed), pulled via pg_get_functiondef before
-- editing, per project convention.
--
-- Also adds providers.total_volume: a plain per-leader counter (not
-- derived from any customer table) that the function below now
-- increments on every closed trade, so trading volume is tracked
-- per-leader independently, the same way base_followers_count already is.
alter table public.providers add column if not exists total_volume numeric not null default 0;
...
-- Drop the child tables first (explicit, even though cascade would
-- handle it) so it's clear from reading this file alone what's being
-- removed: every table that only ever existed to back the "recent
-- copiers" list and its per-fake-customer deposit/withdrawal/pause
-- history, none of which any remaining application code or database
-- function reads after the edit above.
drop table if exists public.synthetic_customer_pauses cascade;
drop table if exists public.synthetic_customer_withdrawals cascade;
drop table if exists public.synthetic_customer_deposits cascade;
drop table if exists public.synthetic_customers cascade;
