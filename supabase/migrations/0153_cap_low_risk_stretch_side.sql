-- Realistic PnL dynamics (part 3): the non-triggering side's "push it out of
-- reach" stretch (1.4x-2.2x) could still carry a low-risk archetype's stored
-- (never-to-be-hit) TP/SL past the 0.5%-3% band -- e.g. a 3% TP stretched
-- 2.2x stores as 6.6%. Low-risk archetypes now use a gentler stretch
-- (1.2x-1.5x) hard-capped at 3.5% of price, so even the displayed,
-- never-triggered side stays within a believable low-risk range.
-- high_risk/struggling are unchanged.
--
-- Live function definition pulled via pg_get_functiondef before editing (per
-- project convention); only the post-TP/SL stretch block changed.
