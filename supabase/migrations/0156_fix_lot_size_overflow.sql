-- Bug fix (pre-existing, unrelated to today's other changes -- surfaced by
-- the total_profit resum in the PnL-realism work): v_lot_size could blow up
-- to an absurd value (one row hit 29 trillion) when v_pips was extremely
-- small (division by a near-zero denominator) -- the existing guard only
-- checked v_pips > 0, not that it was large enough to keep the result sane.
-- Hard-capped at 500 lots (comfortably above the observed p99.9 of ~167).
-- The 79 already-corrupted historical rows (across 54 providers) were fixed
-- separately, and providers.total_profit was re-resummed afterward.
--
-- Live function definition pulled via pg_get_functiondef before editing (per
-- project convention); only the v_lot_size line changed.
  ...
  loop
    ...
    if v_hit_sl then
      ...
      if random() < 0.10 then
        ...
        -- Realistic PnL dynamics (per-leader risk profile): a break-even
        -- close stays within -0.2%..+0.2% for every archetype.
        v_move := 0.0005 + random() * 0.0015;
      else
        v_close_trigger := 'timeout';
        v_skill_rr := greatest(0.6, least(5.0, 3.0 + (coalesce(v_signal.skill, 0.55) - 0.55) * 6.667));

        case v_archetype
          -- Low-risk profiles (flagship/stable/balanced/good_rr): a winning
          -- trade is hard-clamped to 0.5%..3%, matching a disciplined,
          -- modest-target trader -- the skill/rr multiplier still decides
          -- WHERE in that band a leader lands, it just can't escape it.
          when 'flagship' then
            v_base_loss := 0.0015 + random() * 0.0045;
            v_move := case when v_is_win then greatest(0.005, least(0.03, v_base_loss * v_skill_rr)) else v_base_loss end;
          when 'stable' then
            v_base_loss := 0.0015 + random() * 0.0045;
            v_move := case when v_is_win then greatest(0.005, least(0.03, v_base_loss * v_skill_rr)) else v_base_loss end;
          when 'good_rr' then
            v_base_loss := 0.006 + random() * 0.014;
            v_move := case when v_is_win then greatest(0.005, least(0.03, v_base_loss * coalesce(v_signal.rr_ratio, 2.5))) else v_base_loss end;
          when 'high_risk' then
            -- High risk: real risk of losing up to the full account on a
            -- single trade (capped at 100%, not the old 125% theoretical
            -- max) -- the periodic automatic margin call (see the
            -- risk_archetype = 'high_risk' block later in this function)
            -- is what turns this into an actual periodic wipeout, not just
            -- a wide single-trade range.
            v_base_loss := 0.05 + random() * 0.20;
            v_move := case when v_is_win then least(1.00, v_base_loss * v_skill_rr) else v_base_loss end;
    ...
    -- MT5-standard fields: a flat per-lot commission, and an overnight
    -- swap (0 for crypto -- perpetual instruments don't carry a traditional
    -- swap -- and for same-day closes; otherwise a small per-lot-per-day
    -- financing charge/credit, either sign).
    ...
    -- Everything below this line is the LEADER's own performance/follower
    -- flavor (provider stats, synthetic-customer compounding, join/churn
    -- rolls) -- none of it applies when this signal is a created_by_admin
    -- row (a manual admin correction or margin-call settlement -- density
    -- trades are no longer created_by_admin as of this migration, so this
    -- guard now only ever affects genuine admin actions). Those still
    -- close through this exact loop (same TP/SL/timeout logic) so the one
    -- real customer they belong to gets credited via the
    -- close_simulated_positions trigger fired by the UPDATE above, but
    -- they must never be double-counted into the leader's own track
    -- record.
    ...
    qualifying as (
      ...
      where v_pnl > 0 and random() < 0.03
        -- A customer only ever withdraws "their share of profit" -- doesn't
        -- make sense for a leader whose own overall track record is a net
        -- loss, even if this one trade happened to be a winner.
    ...
    if v_is_win and random() < v_join_prob then
      -- Was far too generous across the board (even a p10-skill leader
      -- had a $12k+ ceiling, median leader over $31k) -- a quadratic
      -- curve off a higher skill threshold keeps weak/average leaders
      -- realistic (a few hundred to a few thousand) while still letting
      -- genuinely elite leaders (skill 0.7+) attract some larger
      -- accounts, instead of every leader looking equally "rich".
      v_cap_ceiling := 500 + power(greatest(0, v_signal.skill - 0.35), 2) * 150000;
      v_cap_roll := random();
  ...
  loop
    ...
    if v_provider.risk_archetype = 'high_risk' then
      ...
      if v_margin_call then
        ...
        if found then
          -- Drags this leader's active synthetic customers down with them --
          -- a real copy-trading margin call wipes out the copier's mirrored
          -- position too, not just the leader's own account. Frozen
          -- permanently (copy_status = 'left', distinct from the
          -- resumable 'paused'): no new trade is ever applied to them again
          -- -- resume_roll only ever looks at copy_status = 'paused', so
          -- 'stopped' rows are excluded from it for good, matching a
          -- customer who watched the account blow up and walked away rather
          -- than one who simply ran low on balance.
          with wiped as (
            update public.synthetic_customers
        ...
        -- US30 has no real price feed at all (never has) -- no new
        -- trades on it, ever, in any generation path.
    ...
    -- Real, actively-copying customers should always see a visibly
    -- dense open-positions screen for their leader. Rather than
    -- fabricating separate customer-only positions, this simply makes
    -- the leader trade a bit more when they have real demand for it --
    -- the resulting signal is completely ordinary (not
    -- created_by_admin), so it's genuinely public on the leader's own
    -- profile/stats and mirrors 1:1 to every follower (demo and real
    -- alike) through the existing trigger below, exactly like any
    -- other trade this leader takes. No separate system to keep in
    -- sync with reality.
    --
    -- Gated by a per-subscription randomized 10-30 minute delay since
    -- copy_started_at (deterministic from the subscription's own id, so
    -- it's stable across ticks but varies customer to customer -- no
    -- single fixed wait time to notice) -- a trade appearing the instant
    -- someone clicks "copy" would read as scripted. Same range as the
    -- existing copy_started_at grace period used elsewhere (withdrawal
    -- lock, amount-edit lock), so by the time those locks engage, real
    -- open positions already exist to back them up.
    select exists (
    ...
    if random() < v_open_prob then
      ...
      -- US30 has no real price feed at all (never has) -- no new
      -- trades on it, ever, regardless of which path above picked it
      -- (including a leader's own stored symbol_bias, scrubbed
      -- separately in this same migration).
      if v_symbol = 'US30' then
        v_symbol := 'XAUUSD';
      end if;
      ...
      if not v_market_closed or v_symbol = any(v_crypto_symbols) then
        ...
        -- Realistic PnL dynamics: low-risk archetypes (flagship/stable/
        -- good_rr/balanced) derive their pip targets FROM a percentage of
        -- the actual current price, not a flat pip count shared across every
        -- symbol -- a fixed pip range translates to wildly different %
        -- moves on, say, EURUSD (~1.08) vs BTCUSDT (~85000), which is why a
        -- flat range could blow past the 0.5%-3% bound on some symbols.
        -- high_risk/struggling keep their original flat pip ranges (their
        -- wide day-to-day swings are the point; the periodic automatic
        -- margin call is what bounds high_risk's long-run outcome).
        ...
        case v_provider.risk_archetype
          when 'high_risk' then
            v_tp_pips := 60 + random() * 200;
          when 'struggling' then
            v_tp_pips := 20 + random() * 50;
          else
            -- 0.5%-3% take-profit distance, matching the same win-clamp
            -- used by the fallback (no-real-price-coverage) v_move path.
            v_tp_pips := (v_anchor * (0.005 + random() * 0.025)) / v_pip_size;
        end case;
        case v_provider.risk_archetype
          when 'high_risk' then
            v_sl_pips := 50 + random() * 180;
          when 'struggling' then
            v_sl_pips := 15 + random() * 40;
          else
            -- 0.2%-1% stop distance: a disciplined, modest, controlled loss.
            v_sl_pips := (v_anchor * (0.002 + random() * 0.008)) / v_pip_size;
        end case;
        if v_provider.trading_style = 'scalper' then
        ...
        if v_provider.risk_archetype in ('high_risk', 'struggling') then
          ...
          -- Low-risk archetypes: push the non-triggering side out just
          -- enough that it won't accidentally get hit first, without
          -- letting the STORED (even if never reached) target imply a move
          -- miles outside the 0.5%-3% low-risk band.
          if v_is_win then
