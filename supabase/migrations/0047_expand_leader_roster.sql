-- ============================================================
-- Copy Matrix — the leader roster (103 traders) reads too small
-- for the platform's positioning. This migration roughly triples
-- it by generating ~200 new leaders through the exact same real
-- mechanics as every existing one: a real join date, a real trade
-- history built trade-by-trade from their own skill level (with the
-- same organic mix of regular and struggling performers used before),
-- a total_profit summed honestly from that real history, and a
-- follower count placed with the same tiered, tenure-aware formula
-- already used platform-wide — nothing here is a display-only number.
-- ============================================================
...
begin
  for i in 1..200 loop
    ...
    -- Same tiered, tenure-aware follower formula used platform-wide.
