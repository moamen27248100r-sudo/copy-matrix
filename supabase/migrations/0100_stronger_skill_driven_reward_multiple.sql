-- ============================================================
-- Copy Matrix — strengthen the skill-driven win/loss magnitude tilt.
--
-- 0098 introduced v_skill_rr but kept it deliberately mild
-- (0.5x-2.2x, ~0.7x-1.4x for ordinary skill values) so it read as a
-- gentle lean under the archetype's own character. The customer wants
-- it to read as a real, explicit multiple instead: a leader's WIN
-- should be a clear 2x-5x their own LOSS (not just "a bit bigger"),
-- scaled continuously by skill — e.g. a moderate-risk leader losing
-- ~3% per losing trade should win ~9% per winning trade (3x), while a
-- flagship-tier leader (skill ~0.82-0.84) should reach close to 5x.
-- The archetype's own v_base_loss still sets the ABSOLUTE risk size
-- (low-risk archetypes ~1% loss, higher-risk archetypes bigger); this
-- migration only changes the MULTIPLE applied to the win side.
--
-- Anchored at the platform's own default skill (0.55) -> 3x, so a
-- typical leader matches the customer's own "3% loss / 9% win"
-- example almost exactly. Floor lowered to 0.6 (was 0.5) so leaders
-- with real skill well below the archetype's typical range (rare, but
-- present in the data — observed skill as low as 0.20) lean toward
-- losing slightly more than they win, without duplicating what the
-- dedicated `struggling` archetype already does explicitly and more
-- strongly. good_rr and struggling remain untouched — they already
-- have their own explicit asymmetric mechanics (rr_ratio-driven and
-- hardcoded inverse, respectively).
-- ============================================================
...
begin
  ...
  loop
    ...
    -- Skill-driven win/loss MULTIPLE (not just a gentle lean): anchored
    -- at the default skill 0.55 -> 3x, reaching ~5x near flagship-tier
    -- skill (~0.84) and dropping toward a 0.6x floor for genuinely
    -- low-skill leaders outside the dedicated `struggling` archetype.
