-- Follow-up to 0171 (which retuned v_base_loss/v_move and v_tp_pips/
-- v_sl_pips to the strict 3-tier spec, but left two spots untouched):
-- QA found 11,429 closed signals still outside their tier's bound after
-- 0171 went live, almost all sub-minimum ("too small"), traced to two
-- causes:
--
-- 1. The margin-call block's exit_price formula (both places it closes a
--    signal -- the is_margin_call=true row and, on non-recovery, every
--    other open signal) still used a hardcoded 20%-40% loss, not the
--    high-risk tier's 10%-25% max. Retuned here to 0.10 + random()*0.15.
-- 2. (Fixed separately via a one-off script, not a migration change: 771
--    already-OPEN signals had take_profit/stop_loss stored under the
--    PRE-0171 formula -- hitting that stale stored target bypasses
--    v_base_loss/v_move entirely, since v_hit_tp/v_hit_sl reads the
--    stored value directly. Retargeted them to the new bounds so future
--    closes are already correct; new signals opened after 0171 already
--    used the corrected v_tp_pips/v_sl_pips.)
--
-- Redefined from the current live definition (0171); nothing else in the
-- function changed.
