-- ============================================================
-- Copy Matrix — TP/SL become a real, price-driven close trigger
-- instead of decorative always-null columns. At open time every
-- signal now gets a take_profit and stop_loss price (distance sized
-- by risk_archetype, skewed by the same predetermined_win decision
-- from 0111/0114 so a "win"-destined trade's SL sits proportionally
-- further away than its TP, and vice versa for a "loss"-destined
-- trade -- a lean, not a guarantee). Every tick, the engine checks
-- every open signal's symbol against its live market_prices price;
-- if it has genuinely crossed TP or SL, that closes it right there,
-- at that real price -- which can go against the original lean if
-- the market really moves that way, exactly like a real stop-out.
-- The existing random-lifetime timeout logic becomes the fallback
-- for whatever hasn't hit either level yet (so nothing hangs open
-- forever), still using the predetermined outcome + formula exit
-- exactly as before. close_trigger records which of the two closed
-- it ('tp' / 'sl' / 'timeout' / the pre-existing margin-call path).
--
-- Also: EUR/GBP/JPY only get a real price update once a day from
-- their free source (unlike crypto/gold, which move every ~20s), so
-- TP/SL on those pairs would almost never trigger organically and a
-- live price display for them wouldn't feel alive. Cut them down to
-- ~3% of whatever the existing symbol-selection logic would have
-- picked (was up to 40% via symbol_bias/uniform-fallback), falling
-- back to gold instead -- keeps them technically present without
-- pretending they're tick-active.
-- ============================================================

alter table public.signals
  add column if not exists close_trigger text;
...
begin
  ...
  for v_signal in
    ...
    where s.status = 'open'
      and (
        ...
        or (
          s.take_profit is not null and s.stop_loss is not null
          and exists (
            select 1 from public.market_prices mp
            where mp.symbol = s.symbol
              and (
                (s.side = 'buy' and (mp.price >= s.take_profit or mp.price <= s.stop_loss))
                or (s.side = 'sell' and (mp.price <= s.take_profit or mp.price >= s.stop_loss))
              )
          )
        )
      )
  ...
  loop
    ...
    if v_signal.take_profit is not null and v_signal.stop_loss is not null and v_current_price is not null then
    ...
    if v_close_trigger = 'timeout' and v_signal.trading_style = 'scalper' then
    ...
    if v_provider.risk_archetype = 'high_risk' then
      ...
      if v_margin_call then
        ...
        if not found then
          ...
          insert into public.signals (provider_id, symbol, side, entry_price, exit_price, status, opened_at, closed_at, lot_size, is_margin_call, close_trigger)
          values (
            ...
            'closed', now() - interval '5 minutes', now(), 0.01, true, 'margin_call'
          );
        end if;
