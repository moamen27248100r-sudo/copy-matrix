-- The per-customer withdrawal amount was sized off a fraction of that
-- ONE trade's own dollar pnl (v_pnl-derived "gain"), which is usually a
-- small move relative to the account -- producing unrealistic cents-level
-- withdrawals ($0.32, $1.36...). Sizes off the customer's CURRENT BALANCE
-- (post-trade) instead, matching the same fix just applied to
-- scripts/backfill-synthetic-customers.mjs, so a withdrawal reads as a
-- real investor cashing out a meaningful chunk of what they actually
-- have. Probability lowered from 0.03 to 0.012 to keep cumulative drain
-- over a long trade history reasonable now that each event is bigger.
...
begin
  ...
  loop
    ...
    qualifying as (
      -- Sized off the customer's CURRENT BALANCE post-trade, not a
      -- fraction of this one trade's own (usually small) dollar pnl --
      -- see the migration header comment.
      select id, current_capital from updated
      where v_pnl > 0 and random() < 0.012
    ...
    inserted as (
      ...
      select id, v_signal.provider_id, v_signal.id,
        round((current_capital * (0.04 + random() * 0.11))::numeric, 2),
        now()
      from qualifying
