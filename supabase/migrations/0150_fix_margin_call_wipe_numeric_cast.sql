-- Bug fix (caught in testing before any real margin call fired): round()
-- needs a numeric argument, not double precision -- current_capital *
-- random() produces double precision, so the missing ::numeric cast would
-- have thrown "function round(double precision, integer) does not exist"
-- the first time this branch actually executed live.
