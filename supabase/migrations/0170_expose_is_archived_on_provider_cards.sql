-- 0169 added providers.is_archived but the provider_cards view lists its
-- columns explicitly (not select *), so the new column wasn't visible
-- through it -- the homepage/discover queries need it to filter archived
-- leaders out. Live definition pulled via pg_get_viewdef before editing
-- (per project convention); only the final p.is_archived column added.
