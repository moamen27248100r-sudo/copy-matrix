-- Trade EXECUTION characteristics (how long a position stays open, how
-- far its TP/SL sit) now differentiate by trading_style, not just how
-- OFTEN a leader opens positions (v_open_prob, already style-aware) --
-- per explicit customer spec:
--   scalper          : very short hold (minutes), small pip targets (unchanged, already correct)
--   moderate (day trader): closes same trading day (30 min - 7.5h)
--   sporadic (swing) : holds for days/weeks, wide pip targets
--   session_trader    : closes within a session-length window, like a day trader
--   high_risk / margin-call path: unchanged, already its own distinct 2-7 day window
--
-- Before this, moderate and sporadic shared the exact same close-duration
-- bucket (4-48h) and the exact same TP/SL distance formula -- only their
-- OPEN frequency differed, so a "swing investor" and a "day trader" read
-- identically in their own trade history's per-trade duration/target,
-- which is what the customer flagged.
--
-- Also adds a hard 30-day absolute close deadline regardless of style,
-- TP or SL -- found via investigation that a processing backlog had let
-- ~13% of existing closed trades sit open far longer than their style
-- should ever allow (up to 181 days for a *scalper*); this prevents that
-- from recurring going forward. The existing ~41.7K already-closed
-- outlier trades are fixed retroactively in a companion script
-- (closed_at only -- entry/exit price and pnl are untouched).
