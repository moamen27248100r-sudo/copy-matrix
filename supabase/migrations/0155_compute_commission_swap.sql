-- MT5-standard trade fields (part 2): compute commission and swap at close
-- time for every closing path (normal TP/SL/timeout, and both margin-call
-- variants). Commission: a flat per-lot fee ($3-7/lot). Swap: an overnight
-- financing charge/credit, 0 for crypto (perpetual, no traditional swap) and
-- same-day closes, otherwise scaled by lot_size and days held.
--
