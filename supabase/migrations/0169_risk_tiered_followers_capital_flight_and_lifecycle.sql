-- Three related changes to the live simulation engine (run_market_simulation,
-- redefined here from its current live definition in 0156_fix_lot_size_overflow.sql
-- -- confirmed no later migration redefines it), plus a new daily leader
-- lifecycle job:
--
-- 1. base_followers_count cap now separates high_risk (very few followers,
--    real investors avoid gamblers) from flagship/other low-risk archetypes
--    (the bulk of followers), instead of today's flagship == high_risk cap.
-- 2. A margin call now zeroes base_followers_count immediately (previously
--    only wiped/detached the actual synthetic_customers rows -- the display
--    counter kept whatever value it had until it happened to drift down).
-- 3. A newly-joining live customer's starting capital now scales off the
--    LEADER's own min_copy_amount in fixed proportions (58% at the floor,
--    32% a bit above it, ~10% a "whale" account reserved for flagship
--    leaders only) instead of a skill-derived ceiling untethered from what
--    the leader actually asks customers to put up -- matches
--    scripts/backfill-synthetic-customers.mjs's pickStartingCapital so live
--    joins and backfilled history follow the same rule.
-- 4. New: a single trade that stings hard (>15% of the leader's own
--    account) but stops short of a full margin call now has a chance to
--    make a real chunk (30-60%) of that leader's active copiers bail out at
--    once -- distinct from the existing one-at-a-time churn roll further
--    down, and distinct from the full margin-call wipe (still high_risk
--    only).
--
-- Everything else in the function is unchanged from the live 0156 definition
-- (pulled via pg_get_functiondef before editing, per project convention).
...
-- Autonomous leader lifecycle: not deleted, hidden from discovery.
alter table public.providers add column if not exists is_archived boolean not null default false;
...
begin
  ...
  if random() < 0.25 then
    ...
    case v_country_code
      when 'US' then v_country_first := array['Michael','Emily','James','Jessica']; v_country_last := array['Johnson','Williams','Brown','Davis'];
      when 'DE' then v_country_first := array['Lukas','Anna','Max','Lena']; v_country_last := array['Müller','Schmidt','Weber','Becker'];
      when 'FR' then v_country_first := array['Louis','Emma','Hugo','Camille']; v_country_last := array['Martin','Bernard','Dubois','Girard'];
      when 'ES' then v_country_first := array['Pablo','Lucía','Alejandro','María']; v_country_last := array['García','Martínez','López','Sánchez'];
      when 'IT' then v_country_first := array['Marco','Giulia','Luca','Chiara']; v_country_last := array['Rossi','Russo','Ferrari','Esposito'];
      when 'BR' then v_country_first := array['Lucas','Beatriz','Gabriel','Larissa']; v_country_last := array['Silva','Souza','Oliveira','Pereira'];
      when 'CN' then v_country_first := array['Wei','Mei','Jun','Xin']; v_country_last := array['Zhang','Wang','Li','Chen'];
      when 'IN' then v_country_first := array['Arjun','Ananya','Rohan','Priya']; v_country_last := array['Sharma','Patel','Gupta','Kumar'];
      when 'GB' then v_country_first := array['Oliver','Amelia','George','Isla']; v_country_last := array['Smith','Taylor','Wilson','Evans'];
      when 'MX' then v_country_first := array['Diego','Valentina','Santiago','Camila']; v_country_last := array['Hernández','García','Martínez','López'];
      when 'CA' then v_country_first := array['Liam','Olivia','Noah','Emma']; v_country_last := array['Roy','Gagnon','Tremblay','Clark'];
      when 'AU' then v_country_first := array['Jack','Chloe','William','Olivia']; v_country_last := array['Anderson','Wilson','Taylor','Thompson'];
      when 'TR' then v_country_first := array['Emre','Elif','Mehmet','Zeynep']; v_country_last := array['Yılmaz','Kaya','Demir','Şahin'];
      when 'KR' then v_country_first := array['Ji-woo','Min-jun','Seo-yeon','Joon']; v_country_last := array['Kim','Park','Lee','Choi'];
      else v_country_first := array['Daan','Sanne','Sem','Eva']; v_country_last := array['de Jong','Jansen','de Vries','Bakker'];
    end case;
...
select cron.schedule('run_daily_leader_lifecycle', '11 3 * * *', $$select public.run_daily_leader_lifecycle()$$);
