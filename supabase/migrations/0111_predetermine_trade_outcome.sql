-- ============================================================
-- Copy Matrix — previously the engine rolled win/loss at CLOSE
-- time only, using `random() < skill`. That meant a trade sitting
-- open (and showing a real live price-based % to users via
-- OpenOrdersTable) could be trending one way, then get closed with
-- an outcome rolled fresh at that moment with no relationship to
-- what was shown while it was open.
--
-- Now the outcome is decided at OPEN time (stored on the new
-- `predetermined_win` column) and simply honored when the trade
-- closes later, instead of being re-rolled. Already-open signals
-- from before this migration have `predetermined_win = null` and
-- fall back to the old at-close roll so they aren't left stuck.
-- ============================================================

alter table public.signals
  add column if not exists predetermined_win boolean;
