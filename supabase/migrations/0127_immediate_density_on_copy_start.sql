-- ============================================================
-- Copy Matrix — the density mechanic (0126) was only ever picked up on
-- the next engine tick (every ~20-60s) after a real customer started
-- copying. Clarified requirement: it needs to feel immediate -- the
-- moment a real customer starts copying a leader, the system should
-- act right away, not up to a minute later, and the whole point is
-- that a real, actively-copying customer never has a moment with zero
-- open positions (existing withdrawal-lock logic in
-- src/app/portfolio/withdraw/page.tsx already blocks withdrawal while
-- open positions exist -- this makes that reason actually true at all
-- times instead of relying on the separate 10-minute copy_started_at
-- lock alone).
--
-- Extracted the density-seeding logic out of run_market_simulation()
-- (0126) into its own function, seed_density_for_follower(), so it is
-- ONE implementation called from two places: the engine's per-tick
-- loop (unchanged behavior) and, new here, directly from the
-- "start copying" server action right after the subscription goes
-- active -- so density begins forming before the page even finishes
-- redirecting, instead of waiting for the next cron tick.
--
-- security definer + an auth.uid() guard: a signed-in customer's own
-- RPC call can only ever seed density for themselves (auth.uid() is
-- non-null and checked against the argument); the scheduled engine
-- runs as a direct database connection with no JWT, so auth.uid() is
-- null there and the check is skipped -- unrestricted for that path,
-- exactly like before.
-- ============================================================
...
begin
  ...
  select sub.provider_id, coalesce(sub.allocated_amount, 500)
  into v_provider_id, v_allocated_amount
  ...
  -- capped at 3 per call: a gradual build-up (a few engine ticks, or a
  -- couple of immediate calls if the customer reloads their portfolio
  -- a few times right after starting to copy) rather than a single
  -- burst insert.
...
grant execute on function public.seed_density_for_follower(uuid) to authenticated;
...
begin
  ...
  -- ============================================================
  -- Position density for real, actively-copying customers. Extracted
  -- into public.seed_density_for_follower() (above, same migration)
  -- so the exact same logic can also be called immediately when a
  -- real customer starts copying (src/app/discover/actions.ts), not
  -- just picked up on the next tick -- one shared implementation,
