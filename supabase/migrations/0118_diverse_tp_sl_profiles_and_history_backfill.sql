-- ============================================================
-- Copy Matrix — every trade so far (and, until now, every new one)
-- got BOTH a take_profit and a stop_loss, set at open and never
-- touched retroactively. Real traders vary: some run TP+SL together,
-- some only ever set a target, some only ever set a stop, some run
-- naked and rely on discretion/time. This migration:
--
-- 1. Backfills all ~261K existing CLOSED (non-margin-call) signals
--    with a close_trigger consistent with their ALREADY-RECORDED
--    exit_price (never rewrites realized P&L -- that's baked into
--    total_profit/account_capital/customer compounding everywhere
--    already) -- a signal that won closes 'tp' (take_profit = its
--    real exit_price) or 'timeout' if its rolled profile has no TP;
--    a loser closes 'sl' or 'timeout' symmetrically; anything whose
--    net move is tiny closes 'breakeven'. Whichever of TP/SL wasn't
--    the actual trigger (if present in the rolled profile) is placed
--    on the side that was never reached, at a plausible but never-
--    provably-exact distance -- decorative, like the rest of this
--    platform's synthetic history.
-- 2. Backfills the ~760 still-OPEN signals that predate the TP/SL
--    system with real levels, same profile logic, from their
--    existing entry_price forward.
-- 3. Redefines run_market_simulation() so BOTH the live opener and
--    the live closer follow the same diverse-profile logic going
--    forward: new trades roll one of four profiles (TP+SL / TP-only
--    / SL-only / neither) weighted by trading_style/risk_archetype,
--    and the close loop checks each level independently (previously
--    required both to be non-null to check either), plus a small
--    chance of a genuine breakeven close.
-- ============================================================

with base as (
  select s.id, s.entry_price, s.exit_price, s.side, s.symbol,
    coalesce(p.trading_style, 'moderate') as trading_style,
    coalesce(p.risk_archetype, 'balanced') as risk_archetype
  from public.signals s
  join public.providers p on p.id = s.provider_id
  where s.status = 'closed' and coalesce(s.is_margin_call, false) = false
),
calc as (
  select b.*,
    (case b.symbol
       when 'XAUUSD' then 0.1 when 'EURUSD' then 0.0001 when 'GBPUSD' then 0.0001
       when 'USDJPY' then 0.01 when 'BTCUSDT' then 1 when 'ETHUSDT' then 0.1
       when 'SOLUSDT' then 0.01 when 'BNBUSDT' then 0.1 when 'XRPUSDT' then 0.0001
       else 1 end) as pip_size,
    ((b.exit_price - b.entry_price) / b.entry_price) * (case when b.side = 'sell' then -1 else 1 end) as signed_move,
...
bucketed as (
  select c.*,
    (abs(signed_move) < 0.0008) as is_breakeven,
    (signed_move > 0) as is_win,
    case
      when c.trading_style = 'scalper' then
        case when bucket_roll < 0.60 then 'both' when bucket_roll < 0.75 then 'tp_only' when bucket_roll < 0.90 then 'sl_only' else 'neither' end
      when c.risk_archetype in ('high_risk', 'struggling') then
        case when bucket_roll < 0.30 then 'both' when bucket_roll < 0.45 then 'tp_only' when bucket_roll < 0.70 then 'sl_only' else 'neither' end
      else
        case when bucket_roll < 0.45 then 'both' when bucket_roll < 0.65 then 'tp_only' when bucket_roll < 0.85 then 'sl_only' else 'neither' end
    end as bucket
  from calc c
),
final as (
  select b.*,
    greatest(abs(b.exit_price - b.entry_price) * (1.3 + b.dist_mult * 1.7), b.entry_price * 0.003) as untouched_distance
  from bucketed b
)
update public.signals s
set
  close_trigger = case
    when f.is_breakeven then 'breakeven'
    when f.is_win and f.bucket in ('both', 'tp_only') then 'tp'
    when (not f.is_win) and f.bucket in ('both', 'sl_only') then 'sl'
    else 'timeout'
  end,
  take_profit = case
    when f.bucket not in ('both', 'tp_only') then null
    when f.is_win and not f.is_breakeven then f.exit_price
    else round((f.entry_price + (case when f.side = 'buy' then 1 else -1 end) * f.untouched_distance)::numeric, 4)
  end,
  stop_loss = case
    when f.bucket not in ('both', 'sl_only') then null
    when (not f.is_win) and not f.is_breakeven then f.exit_price
    else round((f.entry_price - (case when f.side = 'buy' then 1 else -1 end) * f.untouched_distance)::numeric, 4)
  end
from final f
where s.id = f.id;
...
with base2 as (
  select s.id, s.entry_price, s.side, s.symbol, s.predetermined_win,
    coalesce(p.trading_style, 'moderate') as trading_style,
    coalesce(p.risk_archetype, 'balanced') as risk_archetype,
    coalesce(p.skill, 0.55) as skill
  from public.signals s
  join public.providers p on p.id = s.provider_id
  where s.status = 'open' and coalesce(s.is_margin_call, false) = false
    and (s.take_profit is null or s.stop_loss is null)
),
calc2 as (
  select b.*,
    (case b.symbol
    ...
    coalesce(b.predetermined_win, random() < b.skill) as v_is_win,
    random() as bucket_roll
  from base2 b
),
bucketed2 as (
  select c.*,
    ...
    (case c.risk_archetype
       when 'flagship' then 15 + random() * 40 when 'high_risk' then 60 + random() * 200
       when 'struggling' then 20 + random() * 50 when 'good_rr' then 25 + random() * 70
       when 'stable' then 20 + random() * 45 else 20 + random() * 50 end) as tp_pips_base,
    (case c.risk_archetype
       when 'flagship' then 15 + random() * 35 when 'high_risk' then 50 + random() * 180
       when 'struggling' then 15 + random() * 40 when 'good_rr' then 15 + random() * 35
       when 'stable' then 18 + random() * 40 else 18 + random() * 45 end) as sl_pips_base
  from calc2 c
),
final2 as (
  select b.*,
    (case when b.trading_style = 'scalper' then 8 + random() * 20 else b.tp_pips_base end) as tp_pips_style,
    (case when b.trading_style = 'scalper' then 8 + random() * 20 else b.sl_pips_base end) as sl_pips_style
  from bucketed2 b
),
skewed as (
  select f.*,
    (case when f.v_is_win then f.sl_pips_style * (1.4 + random() * 0.8) else f.sl_pips_style end) as sl_pips,
    (case when f.v_is_win then f.tp_pips_style else f.tp_pips_style * (1.4 + random() * 0.8) end) as tp_pips
  from final2 f
)
update public.signals s
set
  predetermined_win = coalesce(s.predetermined_win, sk.v_is_win),
  take_profit = case
    when sk.bucket in ('both', 'tp_only')
      then round((sk.entry_price + (case when sk.side = 'buy' then 1 else -1 end) * sk.tp_pips * sk.pip_size)::numeric, 4)
    else null
  end,
  stop_loss = case
    when sk.bucket in ('both', 'sl_only')
      then round((sk.entry_price - (case when sk.side = 'buy' then 1 else -1 end) * sk.sl_pips * sk.pip_size)::numeric, 4)
    else null
  end
from skewed sk
where s.id = sk.id;
...
begin
  ...
  loop
    ...
    set total_profit = total_profit + v_pnl,
        ...
        min_copy_amount = greatest(200, round(
          (min_copy_amount * (1 + (case when v_is_win then 0.004 else -0.004 end))) / 50.0
        ) * 50)
