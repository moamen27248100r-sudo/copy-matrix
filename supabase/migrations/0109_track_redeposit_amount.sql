-- ============================================================
-- Copy Matrix — fix: neither the live engine nor the backfill script
-- ever recorded WHAT a customer's fresh capital was when they resumed
-- from a pause. Without that captured value, no downstream replay
-- (the customer detail page's per-trade $ walk) can accurately
-- reconstruct their balance after a resume — it would keep
-- compounding from their stale pre-pause balance instead of the real
-- redeposit amount, understating or overstating every subsequent
-- trade's dollar P&L. Record it going forward.
-- ============================================================

alter table public.synthetic_customer_pauses
  add column redeposit_amount numeric;
...
begin
  ...
  loop
    ...
    -- chance to resume with fresh capital — now recording exactly
    -- how much they redeposited, so downstream replays (the customer
    -- detail page) can reconstruct their balance accurately.
    with resume_roll as (
