-- ============================================================
-- Copy Matrix — refinement to the density mechanic (0126-0128), per
-- explicit feedback:
--
-- 1. A losing phantom position must not drag on -- if a customer is
--    watching, they should see a trade visibly going against them
--    fairly soon, not a slow multi-hour bleed.
-- 2. A loss must track REAL market movement (a genuine stop-loss level
--    that real market_prices crosses), not a synthetic "just pick a
--    number" formula.
-- 3. Explicitly NOT a single catastrophic loss that margin-calls the
--    whole account or force-closes every other open position -- each
--    phantom position already closes independently (no change needed
--    here, just confirmed: nothing in this migration introduces any
--    cross-position cascade).
--
-- Previously predetermined_win and the take_profit/stop_loss bucket
-- were rolled independently -- a "losing" trade could end up with
-- neither level set, falling through to the close loop's slow
-- synthetic timeout formula instead of a real market-tracked close.
-- Now the outcome is decided FIRST, then the TP/SL structure is built
-- to match it: a winner gets a real take-profit at a normal/patient
-- distance (20-60 pips); a loser gets a real, TIGHT stop-loss (8-20
-- pips) so ordinary market volatility crosses it soon, resolving
-- through the existing close loop's real TP/SL check (checked every
-- tick against live market_prices) rather than the timeout branch.
-- Bonus effect: most phantom trades of BOTH outcomes now resolve via
-- genuine price-level crossing instead of the synthetic formula.
--
-- The continuous top-up behavior (0126) is unchanged -- open position
-- count never hits zero regardless of how individual trades resolve.
-- ============================================================
...
begin
  ...
  for v_density_i in 1..least(v_density_target - v_density_current, 3) loop
    ...
    -- Outcome decided first, then TP/SL built to match it -- a losing
    -- trade always gets a real, TIGHT stop-loss (so it resolves against
    -- genuine market_prices movement, not a slow synthetic formula, and
    -- closes soon rather than dragging on), a winning trade gets a real
    -- take-profit at a normal/patient distance. This also means both
    -- outcomes are now driven by real price crossing a level in the
    -- close loop above, not the timeout branch's synthetic move.
    v_is_win := random() < 0.55;

    if v_is_win then
      v_tp_pips := 20 + random() * 40;
      v_sl_pips := 40 + random() * 50; -- wide: shouldn't realistically hit before tp
      v_bucket := case when random() < 0.8 then 'both' else 'tp_only' end;
    else
      v_sl_pips := 8 + random() * 12; -- tight: resolves quickly against real price
      v_tp_pips := 60 + random() * 60; -- far: shouldn't realistically hit before sl
      v_bucket := case when random() < 0.8 then 'sl_only' else 'both' end;
    end if;
