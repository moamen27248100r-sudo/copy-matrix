-- Adds a real "add funds" (multi-deposit) concept for synthetic copiers,
-- matching the customer's five-scenario copiers-list spec (multi-depositor
-- scenario needs to top up an existing customer's capital, not just seed a
-- starting amount). Mirrors the existing synthetic_customer_withdrawals
-- table/CTE pattern exactly, and swaps the realized-profit/pause-floor
-- basis from starting_capital to total_deposited (= starting_capital + sum
-- of all deposits) so multi-depositors are measured against everything
-- they've actually put in, not just their first deposit.

create table public.synthetic_customer_deposits (
  id uuid primary key default gen_random_uuid(),
  customer_id uuid not null references public.synthetic_customers(id) on delete cascade,
...
create index synthetic_customer_deposits_customer_id_idx on public.synthetic_customer_deposits (customer_id);
create index synthetic_customer_deposits_provider_id_idx on public.synthetic_customer_deposits (provider_id);

alter table public.synthetic_customers add column total_deposited numeric not null default 0;

-- Existing rows: total_deposited starts equal to starting_capital (no
-- extra deposits yet) -- the backfill reseed will set exact values for
-- every leader's customers right after this migration runs.
update public.synthetic_customers set total_deposited = starting_capital;
...
begin
  ...
  loop
    ...
    -- total_deposited (starting_capital + any add-funds events) replaces
    -- starting_capital as the basis for the 8x capital cap and the pause
    -- floor, so a customer who topped up is judged against everything
    -- they've actually put in, not just their first deposit.
    with pre as (
      select id, current_capital, starting_capital, total_deposited,
        round(
    ...
    updated as (
      ...
      set current_capital = greatest(10, least(pre.total_deposited * 8, round((pre.current_capital + pre.gain)::numeric, 2)))
      from pre
      where sc.id = pre.id
      returning sc.id, sc.current_capital, pre.total_deposited
    ),
    qualifying as (
    ...
    final_state as (
      select u.id, u.total_deposited, coalesce(w.current_capital, u.current_capital) as final_capital
      from updated u
    ...
    newly_paused as (
      ...
      from final_state fs
      where sc3.id = fs.id
        and (fs.final_capital <= fs.total_deposited * 0.1 or random() < 0.0003)
      returning sc3.id
    )
    ...
    -- Add-funds event: a net-profitable active customer occasionally tops
    -- up their allocation after seeing the leader perform well (the "ج"
    -- multi-depositor scenario continuing to happen live, not just at
    -- seed time). "current_capital > total_deposited" is a cheap proxy
    -- for "up overall" -- avoids a per-tick historical-withdrawals sum
    -- over a potentially large table for every customer on every signal
    -- close, and is at worst slightly conservative (a big past withdrawal
    -- can make a still-profitable customer read as not-yet-profitable by
    -- this proxy, which only ever means fewer top-ups, never a wrong one).
    with depositing_pool as (
      select id, current_capital, total_deposited from public.synthetic_customers
      where provider_id = v_signal.provider_id
        and joined_at <= v_signal.opened_at
        and copy_status = 'active'
        and current_capital > total_deposited
        and random() < 0.0007
    ),
    new_deposits as (
      insert into public.synthetic_customer_deposits (customer_id, provider_id, amount, occurred_at)
      select id, v_signal.provider_id, round((current_capital * (0.3 + random() * 0.7))::numeric, 2), now()
      from depositing_pool
      returning customer_id, amount
    )
    update public.synthetic_customers sc4
    set current_capital = sc4.current_capital + nd.amount,
        total_deposited = sc4.total_deposited + nd.amount
    from new_deposits nd
    where sc4.id = nd.customer_id;
    ...
    if v_is_win and random() < v_join_prob then
      ...
      insert into public.synthetic_customers (provider_id, display_name, starting_capital, current_capital, total_deposited, joined_at, copy_status)
      values (v_signal.provider_id, v_name_first || ' ' || v_name_last, v_new_customer_capital, v_new_customer_capital, v_new_customer_capital, now(), 'active');
    end if;
