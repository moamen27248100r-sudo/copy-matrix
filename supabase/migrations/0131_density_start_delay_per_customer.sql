-- ============================================================
-- Copy Matrix — regression fix: rebuilding density as genuine leader
-- trades (0130) accidentally dropped the per-customer randomized start
-- delay (0128). Re-confirmed as required: a real customer's density
-- boost should only kick in 10-30 minutes (randomized per subscription,
-- not a fixed number) after they start copying -- not the instant they
-- click "copy". Re-added the same delay gate to the new boost check
-- inside run_market_simulation()'s per-provider loop.
-- ============================================================
