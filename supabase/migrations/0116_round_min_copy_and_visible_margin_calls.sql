-- ============================================================
-- Copy Matrix — two fixes:
--
-- 1. min_copy_amount showed odd-cent-precision numbers like $1,065
--    or $587. Recomputed to the nearest $50 (floor stays $200, itself
--    a multiple of 50), and the live per-trade nudge now rounds to
--    the nearest $50 too -- it still drifts with performance, just
--    in clean $50 steps instead of every few cents.
--
-- 2. A high_risk margin call used to resolve entirely within the
--    same tick it was detected: either force-close whatever was
--    already open, or -- if nothing was open -- insert a synthetic
--    trade that was ALREADY closed, so it never showed up in "open
--    orders" at all. Split into two phases: the tick that detects the
--    margin call now stages a genuinely OPEN position first (old
--    backdated entry, oversized lot, already sitting deep underwater
--    against the current price) so it's actually visible in the open-
--    orders list; the very next tick (~1 minute later, not a normal
--    5-minute-to-7-day hold) finds that staged position and resolves
--    it (force-closed at a big loss, then recover-or-stop as before).
-- ============================================================

update public.providers
set min_copy_amount = greatest(200, round(min_copy_amount / 50.0) * 50);
  ...
  loop
    ...
    if v_provider.risk_archetype = 'high_risk' then
      ...
      if v_margin_call then
        -- Phase 2: a staged margin position from a previous tick is open -- resolve it now
        ...
        -- collateral: any other genuinely open position closes immediately (unrelated to the staged demo trade)
        update public.signals s
        set status = 'closed',
            exit_price = round((
              case when s.side = 'buy' then s.entry_price * (1 - (0.20 + random() * 0.20))
        ...
        -- Phase 1: stage a new, visible margin position -- old backdated entry, oversized lot,
        -- already deep underwater against the current price -- resolved on the NEXT tick
