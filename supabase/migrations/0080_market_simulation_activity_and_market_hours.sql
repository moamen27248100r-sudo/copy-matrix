-- ============================================================
-- Copy Matrix — three realism fixes to run_market_simulation():
--
-- 1. Provider selection for opening trades is now weighted by each
--    provider's activity_weight (0079) using a weighted-without-
--    replacement trick (order by -ln(random())/weight) instead of a
--    flat `order by random()` — so "quiet" traders genuinely end up
--    with a couple of simple trades a day and "power" traders end up
--    opening often, instead of every provider converging to the same
--    average frequency over time.
--
-- 2. A provider with session_start_hour/session_end_hour set only
--    opens trades while the current UTC hour falls in that window
--    (wraps past midnight when start > end); null on both (most
--    providers) still means "trades around the clock."
--
-- 3. Real markets: forex/gold/US30 don't trade weekends or on
--    01-01/12-25, but crypto (BTC/ETH/SOL/BNB/XRP) trades 24/7. So
--    instead of the whole function no-op'ing on those days:
--      - the CLOSING loop only considers non-crypto signals when the
--        market isn't closed; crypto positions keep closing regardless.
--      - the OPENING loop, on a closed-market day, restricts candidate
--        providers to ones who actually carry a crypto symbol in their
--        symbol_bias (so weekend activity isn't "everyone", only
--        traders who genuinely deal in crypto), forces the picked
--        symbol to be crypto, and runs at a lower base probability
--        (thinner weekend activity, not a full weekday's volume).
-- ============================================================
