-- Realistic PnL dynamics (part 2): the position-OPENING TP/SL logic (the
-- primary driver of trade outcomes -- most trades close via a real TP/SL
-- hit, not the v_move fallback tightened in 0151) used a FLAT pip range per
-- archetype, shared across every symbol regardless of price level. That's
-- why a 'balanced' leader could still land a 9%+ move: the same pip count is
-- a tiny % on EURUSD but a large % on a cheap/volatile symbol.
--
-- Low-risk archetypes (flagship/stable/good_rr/balanced) now derive their
-- TP (0.5%-3%) and SL (0.2%-1%) pip distances FROM a percentage of the
-- symbol's actual current price, so the bound holds regardless of which
-- symbol gets picked. high_risk/struggling keep their original flat pip
-- ranges unchanged.
--
-- Live function definition pulled via pg_get_functiondef before editing (per
-- project convention); only the v_target_pips/v_tp_pips/v_sl_pips archetype
-- blocks changed.
