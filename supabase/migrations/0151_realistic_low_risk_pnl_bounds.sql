-- Realistic PnL dynamics (part 1 of the risk-profile overhaul): low-risk
-- archetypes (flagship/stable/balanced/good_rr) now hard-clamp a winning
-- trade to 0.5%-3%, and every archetype's break-even close stays within
-- -0.2%..+0.2% (was up to 0.25%). High-risk keeps its wide day-to-day range
-- (that's its character) but is capped at 100% instead of the old ~125%
-- theoretical ceiling; the periodic automatic margin call (0149/0150) is
-- what delivers the actual "periodic severe wipeout" requirement.
--
-- Live function definition pulled via pg_get_functiondef before editing (per
-- project convention); only the breakeven/archetype v_move block changed.
