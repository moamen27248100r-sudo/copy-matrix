-- ============================================================
-- Copy Matrix — synthetic customers can run out of balance and pause.
--
-- A customer's compounding capital can now cross their own proportional
-- floor (10% of their starting_capital — the same bound the backfill
-- script's per-step walk already uses) or hit a rare independent
-- roll, mirroring the leader's own margin-call mechanic but scaled
-- per-customer and RESUMABLE (not permanent): a paused customer stops
-- participating in compounding/withdrawals entirely until a per-tick
-- resume roll (in the OPEN block, which already scans every provider
-- every minute) gives them fresh capital and reactivates them.
--
-- Both the live engine and the detail page's trade-history replay now
-- skip any leader trade that falls inside a paused window — a paused
-- customer simply didn't "experience" those trades.
--
-- Both `authenticated` and `anon` read policies ship together (the
-- 0104->0105 lesson: don't ship public-browsable data authenticated-only
-- and need a follow-up fix).
-- ============================================================

alter table public.synthetic_customers
  add column copy_status text not null default 'active'
    check (copy_status in ('active', 'paused'));

create table public.synthetic_customer_pauses (
...
create index synthetic_customer_pauses_customer_paused_idx
  on public.synthetic_customer_pauses (customer_id, paused_at);

create index synthetic_customers_paused_idx
  on public.synthetic_customers (provider_id) where copy_status = 'paused';

alter table public.synthetic_customer_pauses enable row level security;

create policy "synthetic_customer_pauses_select_all" on public.synthetic_customer_pauses
  for select to authenticated using (true);

create policy "synthetic_customer_pauses_select_public" on public.synthetic_customer_pauses
...
begin
  ...
  loop
    ...
    -- Synthetic customers: compound, roll withdrawals, then check
    -- whether any of them just crossed into "ran out of balance" —
    -- if so, pause them (they stop participating until resumed by
    -- the OPEN block's per-tick resume roll below).
    with pre as (
    ...
    updated as (
      ...
      set current_capital = greatest(10, round((pre.current_capital + pre.gain)::numeric, 2))
      from pre
      where sc.id = pre.id
  ...
  loop
    -- Give this provider's currently-paused customers a per-tick
    -- chance to resume with fresh capital, before anything else runs
    -- for this provider this minute.
    with resume_roll as (
