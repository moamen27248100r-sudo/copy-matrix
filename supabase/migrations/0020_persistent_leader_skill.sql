-- ============================================================
-- Copy Matrix — the automated simulation was using one flat 55%
-- win probability for every leader's new trades, which over time
-- drags every leader's win rate toward the same mean and erodes
-- the differentiation the ratings system depends on. Give each
-- leader a persistent skill level (seeded from their current
-- track record) and have the simulation use it per-leader.
-- ============================================================

alter table public.providers add column skill numeric not null default 0.55;

update public.providers p
set skill = greatest(0.3, least(0.85, coalesce(perf.win_rate_pct, 55) / 100))
from public.provider_performance perf
where perf.provider_id = p.id;
