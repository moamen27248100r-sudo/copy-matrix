-- Follow-up to 0184: that migration fixed withdrawals being sized off a
-- sliver of a single trade's pnl, but the resulting 4-15%-of-balance
-- range still read as too small/frequent-feeling for real cash-out
-- behavior. Raises the typical range to 15-45% of current balance, and
-- adds an occasional (15% of withdrawal events) "big cash-out" of 60-90%
-- -- a real investor sometimes pulls out most or all of their profit at
-- once, not just a nibble every time. Matches the same change just
-- applied to scripts/backfill-synthetic-customers.mjs.
