-- Pre-existing bug (present since 0114, unrelated to this week's
-- deposits/withdrawals work but found while reviewing it): the "churn"
-- mechanic picks a random active customer and sets copy_status='left' on
-- a losing trade WITHOUT ever logging a synthetic_customer_pauses row.
-- Every other permanent exit (margin call, the drawdown-exit scenario)
-- logs an open-ended pause (resumed_at null), which is what the copier
-- detail page's trade-replay uses to stop rendering further trades for
-- that customer. Without it, a churned-out customer's detail page keeps
-- showing trades as if they never left. Fixed by logging the same
-- open-ended pause row churn already should have had all along.
...
begin
  ...
  loop
    ...
    qualifying as (
      -- Sized off the customer's CURRENT BALANCE post-trade -- see the
      -- migration header comment.
      select id, current_capital from updated
      where v_pnl > 0 and random() < 0.01
        ...
        round((
          current_capital * (
            case when random() < 0.15
              then 0.60 + random() * 0.30
              else 0.15 + random() * 0.30
            end
          )
        )::numeric, 2),
        now()
    ...
    if (not v_is_win) and random() < v_churn_prob then
      ...
      insert into public.synthetic_customer_pauses (customer_id, provider_id, paused_at)
      select id, v_signal.provider_id, now() from churned;
    end if;
  end loop;
