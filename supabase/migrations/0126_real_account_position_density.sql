-- ============================================================
-- Copy Matrix — position density for real, actively-copying customers.
-- Full function body pulled live via pg_get_functiondef right before
-- editing (not reconstructed from memory -- confirmed necessary by
-- 0125's provider_cards mistake earlier today), so this is the exact
-- current engine with two changes:
--
-- 1. The CLOSE loop now skips its leader-level side effects (provider
--    total_profit/followers_count/account_capital, synthetic_customer
--    compounding, join/churn rolls) whenever the closing signal is
--    created_by_admin. This was a latent gap even before today (the
--    3 existing admin-manual-trade signals could theoretically hit
--    it if left open long enough to reach the loop's own timeout),
--    but it becomes load-bearing now that density trades close
--    through this exact loop continuously.
--
-- 2. A new loop generates the density itself: for every real,
--    actively-copying follower (resolved once up front, not a
--    per-provider subquery, so providers with zero real followers --
--    the vast majority of ~1,900+ today -- cost nothing extra per
--    tick), tops up their open phantom-position count toward
--    leader_open_count + 8..19 (always more than the leader, even
--    when the leader has zero open signals), capped at 3 new
--    positions per tick per customer for a gradual ramp rather than a
--    burst insert. Each phantom position is its own signals row
--    (created_by_admin = true, entry price offset by a randomized
--    30/50/100/200-point "delayed entry", backdated opened_at up to
--    45 minutes, the same tp/sl/both/neither risk-bucket roll used
--    for real leader signals) linked to exactly one simulated_positions
--    row for that one customer. On weekends/holidays (v_market_closed)
--    the symbol pool is restricted to crypto only, so density keeps
--    moving through closures that pause forex/gold. Closing is NOT a
--    new mechanism -- these signals flow through the existing
--    TP/SL/timeout CLOSE loop above like any other signal, inheriting
--    that leader's own hold-time/skill personality, and the real
--    customer gets credited through the existing close_simulated_positions
--    trigger (fired by that loop's UPDATE) exactly like a genuine
--    mirrored position would be.
--
-- Prerequisites already shipped ahead of this: hot-path indexes on
-- simulated_positions/subscriptions (0123), and provider_performance
-- excluding created_by_admin signals from public leader stats (0125)
-- -- both specifically because this migration was about to add real
-- write/read volume to exactly those tables/views.
-- ============================================================
...
begin
  ...
  loop
    ...
    -- row (a manual correction, margin-call settlement, or one of this
    -- customer's own density-mechanic phantom positions below). Those
    -- still close through this exact loop (same TP/SL/timeout logic) so
    -- the one real customer they belong to gets credited via the
    ...
    -- record -- that's the entire reason provider_id on them points at a
    -- leader who never actually made the trade.
  ...
  -- ============================================================
  -- Position density for real, actively-copying customers. Real
  -- accounts should always see a visibly dense open-positions screen
  -- -- deliberately exceeding the leader's own open-signal count --
  -- that keeps moving 24/7, including weekends when forex/gold
  -- trading (and so the leader's own signal generation) is dormant.
  --
  -- Implemented as ordinary signals rows (created_by_admin = true,
  -- the same "one specific client's own trade" pattern addClientTrade
  -- already uses) each linked to exactly one simulated_positions row
  -- for that one customer -- never mirrored to anyone else
  -- (mirror_signal_to_followers already returns early on
  -- created_by_admin), excluded from the leader's own public
  -- stats/profile (provider_performance, trader/[id]), and closed by
  -- the exact same TP/SL/timeout logic as any other signal in the
  -- close loop above (inherits that leader's own hold-time/skill
  -- personality since it still carries their provider_id -- but the
  -- "continue" guard added there stops it from ever being counted
  -- into the leader's own performance).
  --
  -- Scoped to real, actively-copying followers only, resolved once up
  -- front instead of a per-provider subquery -- so the ~1,900+
  -- providers with zero real followers cost nothing extra per tick,
  -- and load scales with actual real-customer count, not provider
  -- count.
  -- ============================================================
  for v_density_follower in
    select sub.follower_id, sub.provider_id, coalesce(sub.allocated_amount, 500) as allocated_amount
    from public.subscriptions sub
    join public.profiles fp on fp.id = sub.follower_id
    where sub.is_active = true and fp.account_type = 'real' and fp.balance > 0
  loop
    select count(*) into v_leader_open_count
    from public.signals
    where provider_id = v_density_follower.provider_id and status = 'open' and not created_by_admin;

    select count(*) into v_density_current
    from public.simulated_positions sp
    join public.signals s2 on s2.id = sp.signal_id
    where sp.follower_id = v_density_follower.follower_id
      and sp.status = 'open'
    ...
    if v_density_current < v_density_target then
      -- capped at 3/tick per customer: a gradual build-up to full
      -- density over the first few ticks after they start copying,
      -- not a single burst insert.
      for v_density_i in 1..least(v_density_target - v_density_current, 3) loop
        ...
        -- variable delayed-entry offset: a real copy fill rarely lands
        -- at the exact live tick price -- a randomized point offset
        -- (either direction) keeps that natural instead of every
        -- position opening at a suspiciously identical price.
        ...
        values (
          v_density_follower.provider_id, v_symbol, v_side, v_entry, 'open', v_density_opened_at,
          random() < 0.55, true,
          case when v_bucket in ('both', 'tp_only') then
            (case when v_side = 'buy' then round((v_entry + v_tp_pips * v_pip_size)::numeric, 4)
                  else round((v_entry - v_tp_pips * v_pip_size)::numeric, 4) end)
        ...
        values (
          v_density_signal_id, v_density_follower.follower_id, v_entry,
          round((v_density_follower.allocated_amount * (0.05 + random() * 0.15))::numeric, 2),
