-- ============================================================
-- Copy Matrix — "متداول نشط" is still the smallest-looking figure
-- next to the platform's other real stats. Same honest generator as
-- 0047/0048 (real join dates, real trade-by-trade history driven by
-- skill, honest total_profit, tiered follower counts), adding ~450
-- more real leaders to push the roster well past 1,000.
-- ============================================================
...
begin
  for i in 1..450 loop
    ...
    -- Skewed toward more recent join dates than earlier batches, so
    -- this round's leaders naturally carry shorter (but still real)
    -- trade histories instead of compounding the platform's total
    -- signal volume as heavily as the first two expansions did.
    v_joined := timestamp '2023-06-01' + (random() * (extract(epoch from (now() - timestamp '2023-06-01')) / 86400) || ' days')::interval;
    ...
    v_trade_count := greatest(8, least(200, round(v_tenure_days * 0.18 * (0.7 + random() * 0.6))::int));
    v_pnl_sum := 0;
