-- ============================================================
-- Copy Matrix — two fixes prompted by the same conversation:
--
-- 1. US30 has no real price feed at all (documented, accepted gap
--    since 0072 -- no free key-less source exists for it). Explicit
--    request: stop generating any new US30 trades, and close out the
--    41 that are currently open (they can never get a real live price
--    to track). Both symbol-selection paths in the engine (the
--    margin-call synthetic trade and the main per-provider open
--    block, which covers normal trades, the XAUUSD-forced 60% roll,
--    AND a leader's own symbol_bias-driven pick) now redirect US30 to
--    XAUUSD instead. Leaders' stored symbol_bias arrays are scrubbed
--    the same way (array_replace, not removed -- keeps the array's
--    positional indexing, which the weighted-tier pick logic depends
--    on, intact). The 25,759 already-CLOSED historical US30 trades
--    are left alone -- they're already resolved/settled with their
--    own recorded entry/exit prices (same as any symbol whenever it
--    resolves via the timeout branch rather than a real TP/SL cross,
--    which happens routinely platform-wide), not something "cancel"
--    meaningfully applies to; only genuinely open, un-trackable
--    positions are force-closed here.
--
-- 2. Checked before answering "can we fetch faster than 20s safely":
--    net._http_response (the table whose runaway growth caused
--    today's earlier incident) has had ZERO cleanup since it was
--    introduced in 0072 -- it grows by ~2 rows every fetch cycle,
--    forever, at ANY cadence. This was always going to eventually
--    cause the same class of problem again, just slower at 20s than
--    it did at 1s. Added a cron job that prunes rows older than 1
--    hour (comfortably past the sub-second read latency
--    apply_price_fetch_responses actually needs) every 10 minutes --
--    the real fix that makes a future cadence increase safe to even
--    consider, independent of whatever cadence is chosen.
-- ============================================================

update public.providers
set symbol_bias = array_replace(symbol_bias, 'US30', 'XAUUSD')
where 'US30' = any(symbol_bias);

-- Backdate currently-open US30 signals far enough that the very next
-- engine tick's existing close loop picks them up through its normal
-- timeout logic -- reuses the same tested TP/SL/timeout close path
-- instead of a one-off closing routine.
update public.signals
set opened_at = now() - interval '10 days'
where symbol = 'US30' and status = 'open';

create or replace function public.cleanup_http_responses()
returns void
language sql
security definer
...
select cron.schedule('cleanup-http-responses', '*/10 * * * *', $$select public.cleanup_http_responses();$$);
