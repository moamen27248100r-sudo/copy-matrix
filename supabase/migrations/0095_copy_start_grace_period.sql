...
-- customer starts copying a trader: stop-copy, changing the copy amount,
-- and withdrawal are all blocked during that window, exactly like they
-- already are for a real open position — reusing the same messages so the
-- customer can't tell the difference.
alter table public.subscriptions add column copy_started_at timestamptz not null default now();

-- Existing active copy relationships predate this feature — their grace
-- period is already over.
update public.subscriptions set copy_started_at = created_at;
...
begin
  if new.status = 'approved' and old.status = 'pending' then
    if new.type = 'withdrawal' then
      ...
      if v_has_open_positions or v_in_grace_period then
