-- ============================================================
-- Copy Matrix — 0043 tied follower counts to real performance,
-- but the tenure x skill formula still produced numbers in the
-- thousands for long-tenured leaders. This is a small, early-stage
-- unlicensed platform, not an established one with millions of
-- users — a normal leader here should realistically have a small
-- handful of copiers, up to roughly a dozen or so even for a
-- strong performer, not thousands regardless of how good their
-- win rate is.
--
-- This migration:
--   1. Replaces the follower formula with small, bounded tiers by
--      skill (struggling: 1-5, mid: 3-8, good: 5-12, flagship:
--      10-25), placed within the tier by tenure + noise instead of
--      growing open-endedly with tenure.
--   2. Tones down the live simulation's per-trade follower delta
--      (rarer, +/-1 only) and hard-caps the result so counts stay
--      calm indefinitely instead of drifting back up over time.
-- ============================================================
...
begin
  ...
  loop
    ...
    if v_is_flagship then
      v_tier_min := 12; v_tier_max := 25;
    elsif v_provider.skill >= 0.65 then
      v_tier_min := 5; v_tier_max := 12;
    elsif v_provider.skill >= 0.50 then
      v_tier_min := 3; v_tier_max := 8;
    elsif v_provider.skill >= 0.35 then
      v_tier_min := 2; v_tier_max := 6;
    else
      v_tier_min := 1; v_tier_max := 4;
    end if;

    v_tenure_norm := least(1, v_tenure_days / 900.0);
    v_noise := 0.75 + random() * 0.5;
    v_new_followers := greatest(1, round(v_tier_min + (v_tier_max - v_tier_min) * v_tenure_norm * v_noise)::int);
    v_new_followers := least(v_new_followers, (v_tier_max + 3)::int);
...
begin
  ...
  loop
    ...
    -- Rare, single-follower moves — enough to feel alive without
    -- ever drifting back into large numbers.
    v_follower_delta := case
      when v_is_win and random() < (case when v_is_flagship then 0.05 else 0.02 end) then 1
      when (not v_is_win) and v_is_struggling and random() < 0.04 then -1
      when (not v_is_win) and (not v_is_struggling) and random() < 0.01 then -1
      else 0
    end;
