-- ============================================================
-- Copy Matrix — customer capital and min_copy_amount review, per
-- explicit feedback: synthetic customer capital was too high across
-- the board, and min_copy_amount wasn't clearly tied to a leader's
-- actual performance (neither uniformly high nor low, scaled by
-- skill).
--
-- Root cause for capital: the live engine's new-customer capital
-- ceiling was `5000 + max(0, skill-0.2) * 85000` -- even a weak,
-- p10-skill leader (skill ~0.29) had a $12,650 ceiling, and the
-- MEDIAN leader (skill ~0.51) had a $31,350 ceiling. Checked live:
-- synthetic_customers.starting_capital was barely correlated with
-- leader rating at all (rating 30-39 averaged HIGHER customer capital
-- than rating 70-79) and had a $52k max. Replaced with a steeper,
-- quadratic curve off a higher skill threshold (500 + max(0,
-- skill-0.35)^2 * 150000) -- weak/average leaders now cap out at a
-- few thousand, only genuinely elite leaders (skill 0.7+) unlock
-- larger accounts. Existing 188K customer rows are rescaled
-- proportionally per leader (old ceiling -> new ceiling ratio) by
-- scripts/rescale-synthetic-customer-capital.mjs, run right after
-- this migration -- preserves each customer's relative standing and
-- their full compounding history (starting_capital and current_capital
-- scaled by the same factor), just brings the absolute dollar scale
-- down to something realistic.
--
-- min_copy_amount: already correlated with skill somewhat (avg $444
-- at rating 10-19 vs $1092 at rating 70-79) but the range was narrow
-- ($200-1350) and non-monotonic at the low end. Recomputed from skill
-- directly with a wider, cleaner curve and re-applied to every
-- existing provider in this same migration (a direct UPDATE, not a
-- separate script -- much smaller change, no compounding history to
-- preserve).
-- ============================================================

update public.providers
set min_copy_amount = greatest(50, round((50 + power(greatest(0, skill - 0.2), 1.3) * 3000) / 50) * 50)
