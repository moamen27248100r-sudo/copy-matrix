-- ============================================================
-- Copy Matrix — synthetic customers get real withdrawal history.
--
-- New public.synthetic_customer_withdrawals table: on a subset of a
-- provider's winning trade closes, a small % of the qualifying
-- synthetic customers "cash out" part of their gain — mirroring the
-- leader's own v_withdrawal_bump mechanic (10-40% of the gain), but
-- at a much lower per-customer rate (3%, since many customers roll
-- independently per trade vs. one roll for the leader).
--
-- Both `authenticated` and `anon` read policies ship together this
-- time (0104 shipped synthetic_customers with only `authenticated`
-- and needed a follow-up fix, 0105, after guests got an empty list).
-- ============================================================

create table public.synthetic_customer_withdrawals (
...
create index synthetic_customer_withdrawals_customer_occurred_idx
  on public.synthetic_customer_withdrawals (customer_id, occurred_at);

alter table public.synthetic_customer_withdrawals enable row level security;

create policy "synthetic_customer_withdrawals_select_all" on public.synthetic_customer_withdrawals
  for select to authenticated using (true);

create policy "synthetic_customer_withdrawals_select_public" on public.synthetic_customer_withdrawals
...
begin
  ...
  loop
    ...
    -- capital (same % basis v_pnl was computed against), and a small
    -- subset of customers with a positive gain on this trade
    -- probabilistically cash some of it out into a withdrawal-events
    -- ledger — mirrors v_withdrawal_bump above, but at a much lower
    -- per-customer rate since many customers roll independently per
    -- trade (vs. one roll for the leader itself).
    with pre as (
