-- ============================================================
-- Copy Matrix — automated margin-call cycle for high_risk leaders.
--
-- Previously the only "margin call" concept for a LEADER's own
-- account (as opposed to a customer's copy-wallet, or the admin's
-- manual per-customer tool) was a one-off cosmetic bad-trade planted
-- into ~35% of high_risk leaders' seed history (migrations 0055-
-- 0057) — no persistent state, nothing ongoing.
--
-- This adds a real, permanent mechanic: an active high_risk leader
-- can blow their account (organically, via account_capital drifting
-- below $200, or via an independent low-probability per-tick roll —
-- ~once/month on average per active leader), after which they either
-- redeposit and continue (odds decreasing with repeat occurrences:
-- 70% / 50% / 25%) or permanently stop trading. Stopped leaders are
-- skipped by the OPEN block forever after; their existing followers
-- simply stop receiving new copied trades automatically (the existing
-- on_signal_created trigger only mirrors NEW open signals), and their
-- historical data/profile stays fully visible — nothing here deletes
-- or hides past data.
--
-- The pre-existing admin addMarginCallTrade tool (a manual, customer-
-- scoped single-trade action) is untouched and shares no state with
-- this mechanic.
-- ============================================================

alter table public.providers
  add column if not exists trading_status text not null default 'active'
    check (trading_status in ('active', 'stopped')),
  add column if not exists margin_called_at timestamptz,
  add column if not exists margin_call_count int not null default 0;

alter table public.signals
  add column if not exists is_margin_call boolean not null default false;
...
begin
  ...
  loop
    -- Leaders who permanently stopped after a margin call never trade
    -- again. Their existing followers simply stop receiving new
    -- copied trades automatically (on_signal_created only mirrors NEW
    -- open signals) — no other code needs to change for that.
    if v_provider.trading_status = 'stopped' then
      continue;
    end if;

    -- Automated margin-call cycle: only high_risk leaders (matching
    -- their archetype's identity — big leveraged swings). Triggered
    -- either organically (account_capital already bled down near the
    -- 0102 division-guard floor) or by an independent, rare per-tick
    -- roll standing in for a sudden leveraged blowup. ~0.000023 per
    -- minute -> ~once/month mean per active high_risk leader.
    if v_provider.risk_archetype = 'high_risk' then
      ...
      if v_margin_call then
        ...
        if not found then
          -- nothing was open to force-close — plant one synthetic
          -- closed margin-call trade so the event is still visible in
          -- this leader's history (same spirit as the one-off 0055
          -- cosmetic pattern, but now flagged and persistent).
        ...
        if random() < v_recover_prob then
          -- redeposit: fresh capital in high_risk's own seed range
          -- (RANGES.high_risk in scripts/assign-provider-capital.mjs —
          -- keep these two numbers in sync if that range ever changes)
          update public.providers
          ...
          -- permanently stop: leave a small non-zero "dust" residual,
          -- never trades again (caught by the guard at the top of
          -- this loop on every future tick).
          update public.providers
        ...
        continue; -- don't also open a fresh trade the same tick the account just blew up
      end if;
    end if;
