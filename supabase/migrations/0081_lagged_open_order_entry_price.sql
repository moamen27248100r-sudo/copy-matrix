-- ============================================================
-- Copy Matrix — new "open orders" (signals the automated engine opens
-- for a leader) no longer enter at the exact instantaneous live price.
-- That would mean the platform is "predicting" the market at the exact
-- moment it opens a trade, which isn't realistic and makes win/loss
-- look suspiciously perfectly timed.
--
-- Instead: pick a random lookback (varies every trade, no fixed
-- amount) and pull the REAL price from price_history at that point in
-- the past as the entry price — so the trade always looks like it's
-- reacting to a market move that already happened (a lagged/delayed
-- entry, like real copy-trading execution latency), not entering at a
-- perfectly-synced live tick. The win/loss decision itself is still
-- made later at close time from real price direction (unchanged) —
-- this only changes where the entry price comes from.
--
-- Falls back to the old live-price-with-jitter behavior when a symbol
-- has no real price_history coverage that far back (US30, or a
-- genuine gap), so the engine never stalls.
--
-- This is scoped ONLY to the automated engine's own trade generation.
-- The admin margin-call feature and the other manual admin trade tools
-- (createTraderTrade/addClientTrade) are deliberate admin actions and
-- stay exact/live, unchanged.
-- ============================================================
...
begin
  ...
  v_market_closed := (v_dow = 0 or v_dow = 6 or v_month_day in ('01-01', '12-25'));
    ...
    loop
      ...
      if v_in_session and random() < 0.4 then
        ...
        if not v_market_closed or v_symbol = any(v_crypto_symbols) then
          ...
          -- Lagged entry: pick a random lookback (no fixed amount —
          -- varies every trade) and use the REAL price from that point
          -- in the past, not the live tick. Reads as "the system
          -- followed a move that already happened" rather than
          -- predicting the market.
          v_lag_minutes := 3 + floor(random() * 57)::int; -- 3 to 60 minutes
          select price into v_lagged_entry
          from public.price_history
          where symbol = v_symbol and ts <= now() - (v_lag_minutes || ' minutes')::interval
          order by ts desc limit 1;

          if v_lagged_entry is not null then
            v_entry := v_lagged_entry;
          else
            -- No real coverage that far back (US30, or a genuine gap)
            -- — fall back to the previous live-price-with-jitter entry.
