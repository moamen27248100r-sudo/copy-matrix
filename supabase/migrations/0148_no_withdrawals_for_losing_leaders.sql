-- Withdrawal mechanic no longer fires for a leader whose overall track
-- record is a net loss, even on an individual winning trade close. Matches
-- the one-time cleanup that removed the existing inconsistent withdrawal
-- history for losing leaders' customers (73,125 rows, refunded to
-- current_capital) -- this keeps it from re-accumulating.
--
-- Live definition pulled via pg_get_functiondef before editing (per project
-- convention); only the `qualifying` CTE's WHERE clause changed.
