-- Retunes run_market_simulation()'s per-trade PnL bounds to a strict
-- 3-tier spec (customer's explicit numbers), redefined from the current
-- live definition in 0169_risk_tiered_followers_capital_flight_and_lifecycle.sql
-- (confirmed no later migration redefines the function; the lifecycle
-- function/cron job from 0169 is untouched and not re-declared here).
--
-- Two localized edits, both keyed off the same risk_archetype used
-- everywhere else in this function (no new columns -- max_single_trade_pnl/
-- min_single_trade_pnl/volatility_factor would be fully redundant with it):
--
-- 1. The CLOSE loop's v_base_loss/v_move case block (realized outcome):
--      low  (flagship/stable/good_rr/else): win 0.4%-2.5%, loss 0.3%-1.8%
--      moderate (struggling):                win 1.0%-4.5%, loss 0.8%-3.5%
--      high (high_risk):                     win 5%-15% (was uncapped to
--                                             100%), loss 10%-25%
-- 2. The OPEN block's v_tp_pips/v_sl_pips case block (stored target,
--    never necessarily hit) retuned to the same bounds, and switched from
--    high_risk/struggling's old flat pip-count ranges to the same
--    price-relative formula low-risk already used -- a flat pip range
--    meant wildly different % moves across symbols (BTC ~$85k vs EURUSD
--    ~$1.08); now every tier is consistent.
