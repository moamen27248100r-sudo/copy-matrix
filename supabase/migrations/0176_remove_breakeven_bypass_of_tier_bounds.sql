-- Final QA pass found the true root cause behind every remaining
-- out-of-bound closed signal (including several that kept appearing
-- minutes after 0174 and 0175 were both live and confirmed working):
-- a pre-existing, tier-system-unaware branch that, 10% of the time a
-- signal closes without hitting its stored TP/SL, labeled the close
-- 'breakeven' and forced v_move to a flat 0.05%-0.2% -- below every
-- tier's floor (low tier's own floor is 0.3%/0.4%), regardless of the
-- leader's risk_archetype. Unlike the scalper-specific overrides fixed
-- in 0175, this one hit EVERY leader, all trading styles alike -- it's
-- what caused the ~590 lingering "breakeven" outliers seen throughout
-- this QA pass and the small trickle still appearing on production
-- after every other fix landed.
--
-- 'breakeven' is not surfaced anywhere in the UI (confirmed via a full
-- grep of src/) -- it was purely an internal close_trigger label with
-- no downstream meaning, so removing it entirely is safe. Every
-- non-TP/SL close now always goes through the same tier-based
-- v_base_loss/v_move case block (close_trigger := 'timeout'), so 100%
-- of closes are tier-consistent with zero remaining exceptions.
--
-- Redefined from the current live definition (0175); nothing else
-- changed.
