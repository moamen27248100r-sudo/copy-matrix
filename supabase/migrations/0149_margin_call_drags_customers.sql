-- Margin calls on a high_risk leader now drag their active synthetic
-- customers down with them (severe capital cut + permanently marked 'left',
-- the same status the existing voluntary-churn mechanic already uses, so no
-- schema change is needed -- 'left' is already excluded from both the
-- resume-roll and the trade-application CTE).
--
-- Live function definition pulled via pg_get_functiondef before editing (per
-- project convention); only the margin-call block changed (see comment
-- inline).
    ...
    if v_provider.risk_archetype = 'high_risk' then
      ...
      if v_margin_call then
        ...
        if found then
          ...
          with wiped as (
            update public.synthetic_customers
            set current_capital = round(current_capital * (0.02 + random() * 0.08), 2),
                copy_status = 'left'
