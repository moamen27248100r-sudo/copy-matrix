-- ============================================================
-- Copy Matrix — fix: the scalper pip cap introduced in 0098 only
-- applied on the real-price path (`v_used_real`). Live verification
-- right after deploying 0098 caught scalper trades on BTCUSDT/US30
-- (pip_size = 1, high nominal price) resolving at 200-300+ "pips" via
-- the FABRICATED fallback path — a percentage move that's small in %
-- terms still translates into a huge raw price delta for those two
-- symbols. The cap now applies unconditionally whenever
-- trading_style = 'scalper', regardless of which path produced
-- v_exit_price. Everything else is unchanged from 0098.
-- ============================================================
...
begin
  ...
  loop
    ...
    -- FIX (0099): cap applies regardless of which path produced
    -- v_exit_price — the fabricated path's percentage move can still
    -- translate into a huge raw price delta for pip_size=1, high
    -- nominal-price symbols (BTCUSDT, US30).
    if v_signal.trading_style = 'scalper' then
