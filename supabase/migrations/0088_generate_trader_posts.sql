-- Daily "trader posts" feed generation. Same idiom as mirror_signal_to_followers()
-- / addMarginCallTrade(): hand-composed Arabic template text built from a
-- REAL closed signal that trader actually has — never fabricated advice,
-- never sourced from outside the platform.
...
begin
  ...
  loop
    ...
    insert into public.trader_posts (provider_id, signal_id, body)
    values (v_provider.id, v_signal.id, format(v_template, v_side_label, v_symbol_ar, round(abs(v_pct), 2)::text));
  end loop;
...
select cron.unschedule(jobid) from cron.job where command = 'select public.generate_trader_posts();';
select cron.schedule('0 8 * * *', 'select public.generate_trader_posts();');
