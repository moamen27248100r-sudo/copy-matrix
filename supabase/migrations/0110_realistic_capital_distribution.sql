-- ============================================================
-- Copy Matrix — fix: customer starting capital was drawn log-uniform
-- between the leader's floor and ceiling, which gives EQUAL weight to
-- every order of magnitude (as many customers at $10k-60k as at
-- $100-600) — not how real retail wealth is distributed. Measured
-- live: 24.6% of customers started above $10k, p99 = $41,351, and
-- current_capital reached as high as $782,396 for some. Fixed in the
-- backfill script (tiered draw: 60% under $800, up to only 0.5% ever
-- reaching a "whale" tier capped by the leader's own rating-based
-- ceiling) and the per-step compounding bound (15x -> 8x of starting
-- capital).
--
-- This migration adds the matching ceiling to the LIVE engine's
-- ongoing compounding — previously only a floor (greatest(10, ...))
-- existed, so a customer's capital could keep growing unboundedly
-- forever. Now capped at 8x their own starting_capital, same as the
-- backfill's per-step bound, so realistic figures stay realistic
-- going forward, not just at seed time.
-- ============================================================
