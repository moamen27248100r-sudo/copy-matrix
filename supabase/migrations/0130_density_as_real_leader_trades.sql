-- ============================================================
-- Copy Matrix — architectural correction to the density mechanic
-- (0126-0129), per explicit feedback: a customer who checks the
-- leader's public profile must see a matching trade for anything open
-- on their own account, or the whole illusion breaks. Fabricating
-- customer-only "phantom" signals (created_by_admin = true, hidden
-- from the leader's public stats/profile) was the wrong shape.
--
-- Replaced entirely: density is no longer a separate parallel
-- trade-generation system. Instead, a provider with real, actively-
-- copying followers whose own open-signal count is below a small
-- randomized floor (6-13) gets its ordinary open-probability boosted
-- this tick. The resulting signal is a completely normal one (not
-- created_by_admin) -- genuinely public on the leader's profile,
-- counted in provider_performance/win_rate like any other trade, and
-- mirrored 1:1 to every follower (demo and real alike) through the
-- existing mirror_signal_to_followers trigger, unchanged. No parallel
-- system to keep in sync with what's real.
--
-- Confirmed with the user: this symmetric (same %, leader and
-- follower) behavior is for ROUTINE density only. The asymmetric
-- "small loss at the leader, big loss at the real customer" case is
-- specific to margin-call-style trades (src/app/admin/actions.ts,
-- still an admin-triggered mechanic today, not this engine) and is
-- handled separately, not here.
--
-- public.seed_density_for_follower() (0127-0129) is now dead code --
-- nothing calls it (the immediate-trigger call in
-- src/app/discover/actions.ts was already reverted in 0128) -- dropped.
-- ============================================================

drop function if exists public.seed_density_for_follower(uuid);
