-- ============================================================
-- Copy Matrix — the market simulation generated entry/exit
-- prices with unrounded floating precision (e.g.
-- 143.6510616851365307462720765706), which then displayed raw
-- and broken-looking in the UI. Round prices going forward and
-- clean up existing data.
-- ============================================================

update public.signals
set entry_price = round(entry_price, 4)
where entry_price != round(entry_price, 4);

update public.signals
set exit_price = round(exit_price, 4)
where exit_price is not null and exit_price != round(exit_price, 4);
...
begin
  ...
  loop
    if random() < 0.4 then
      ...
      v_entry := round(v_base_prices[v_idx] * (1 + (random() - 0.5) * 0.01), 4);
