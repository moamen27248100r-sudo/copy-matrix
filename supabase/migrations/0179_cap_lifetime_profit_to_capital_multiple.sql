-- Follow-up to 0178 (withdrawal targeting): fixing withdrawals exposed
-- that total_profit itself had no ceiling at all. QA found 301 profitable
-- leaders with total_profit more than 20x their current account_capital
-- (140 over 100x, one at 2046x -- $5.37M profit on $4,147 capital), with
-- ~zero correlation to tenure (0.012) -- runaway compounding, not a
-- defensible "years of proven trading" story. Retroactively capped via
-- scripts/cap-unrealistic-leader-profit.mjs (551/1016 profitable leaders
-- needed it) and total_withdrawals re-tightened wherever it would have
-- exceeded the new, lower total_profit.
--
-- This keeps it from drifting back: total_profit now clips at an
-- archetype-scaled multiple of CURRENT account_capital (disciplined
-- low-risk 10-12x, high_risk 18x, struggling 6x) once a WINNING trade
-- would push it past that ceiling. Losses always still apply in full --
-- this only ever clips further upside, never prevents profit from
-- correctly decreasing, and the ceiling itself keeps rising as capital
-- genuinely grows.
--
-- Live definition pulled via pg_get_functiondef before editing; only the
-- total_profit expression in the per-trade UPDATE changed (new
-- v_profit_ceiling variable + case expression).
...
begin
  ...
  loop
    ...
    -- Lifetime total_profit had no ceiling at all -- QA found 301
    -- profitable leaders with total_profit more than 20x their current
    -- account_capital (140 over 100x, one at 2046x: $5.37M profit on
    -- $4,147 capital), with ~zero correlation to tenure, i.e. runaway
    -- compounding rather than a defensible "years of proven trading"
    -- story. Retroactively capped via scripts/cap-unrealistic-leader-
    -- profit.mjs; this keeps it from drifting back. Once total_profit
    -- would exceed a plausible multiple of CURRENT capital (archetype-
    -- scaled: disciplined low-risk lower, high-risk higher variance),
    -- further gains stop accumulating into the displayed lifetime total
    -- until capital itself grows the ceiling back up, or a loss brings
    -- the running total back under it naturally -- losses always still
    -- apply in full, this only ever clips further upside.
